...
class T5(nn.Module):
    ...
    def initialize_weights(self):
        ...
        self.register_buffer('enc_attn_mask', torch.ones(self.max_len, self.max_len)[None, None, ...])
        self.register_buffer('dec_attn_mask', torch.tril(torch.ones(self.max_len, self.max_len))[None, None, ...])
    ...
    def forward_loss(self, target_s, target_a, next_obs, pred_s, pred_a, mask):
...
class T5Agent:
    def __init__(self,
        ...
        # models
        self.model = T5(obs_shape[0], action_shape[0], transformer_cfg).to(device)
        self.mask_ratio = mask_ratio
        self.new_mask_ratio = new_mask_ratio
        self.mask_type = mask_type
        print("agent.mask_type:",mask_type)
        ...
        if mask_utils.curriculum_true(mask_type):
            self.teacher = mask_utils.teacher_init(mask_type,teacher_gamma)
    ...
    def update_mdp(self, states, actions,next_states,current_step=None,total_step=None):
        metrics = dict()
        ...
        #mask_loss, state_loss, action_loss = self.model.forward_loss(states, actions,next_states, pred_s, pred_a, mask)
        ...
        #else:
        #    raise NotImplementedError
        loss = self.model.forward_loss(states, actions,next_states, pred_s, pred_a, mask)
    ...
    def eval_validation(self, val_iter, num_eval,step=None,mask_ratio=0.5,mask_type='fixed_seq_masking2',mask_len=1):
        ...
        for i in range(num_eval):
            batch = next(val_iter)
            obs, action, _, _, next_obs, _ = utils.to_torch(
                batch, self.device)
            #mask_ratio = np.random.choice(self.mask_ratio)
            latent, mask, ids_restore = self.model.forward_encoder(obs, action, mask_ratio,mask_type,mask_len)
            pred_s, pred_a = self.model.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
            #mask_loss, state_loss, action_loss = self.model.forward_loss(obs, action, next_obs,pred_s, pred_a, mask)
            loss= self.model.forward_loss(obs, action, next_obs,pred_s, pred_a, mask)
        ...
        if mask_utils.curriculum_true(self.mask_type):
            ...
            if len(self.hist_rewards) >= 2:
                ...
                mode,num_sub_tasks = self.teacher.info
                if mode=='single':
                    self.teacher.update(self.current_task, scaled_reward)
                #elif mode=='single_random_ratio':
                #    mask_len,mask_ratio = self.current_task
                #    current_arm = (mask_len,0)
                #    self.teacher.update(current_arm, scaled_reward)
                #else:
                #    mask_len,mask_ratio = self.current_task
                #    current_arm = (int((mask_len-1)/num_sub_tasks)+1,mask_ratio)
                #    self.teacher.update(current_arm,scaled_reward)
                else:
                    raise NotImplementedError
        ...
        if self.use_tb:
            ...
            metrics['val_total_loss'] = total_loss/num_eval
    ...
    def update(self, replay_iter, current_step=None,total_step=None):
        ...
        # update critic
        if self.mask_type=='random':
            metrics.update(self.update_mdp(obs, action,next_obs))
        else:
            assert current_step!=None and total_step!=None,"current_step!=None and total_step!=None"
            metrics.update(self.update_mdp(obs, action,next_obs,current_step,total_step))
