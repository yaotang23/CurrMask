...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim, attention_length, finetune, config):
        ...
        self.t5 = T5(obs_dim, action_dim, config)
    ...
    def freeze_layers(self):
        ...
        #     for b in self.t5.encoder_blocks:
    ...
    def forward(self, obs_seq, std):
        if len(obs_seq.size())==2:
            #print()
            batch_size, obs_dim = obs_seq.size()
            obs_seq = obs_seq.view(batch_size,1,obs_dim)
            #print("after unsqueeze:",obs_seq.size())
        batch_size, T, obs_dim = obs_seq.size()
        x = self.t5.state_embed(obs_seq) + self.t5.pos_embed[:, :T]
        # into attention
        for blk in self.t5.encoder_blocks:
            x = blk(x, self.attn_mask)
        #x = self.t5.encoder_norm(x)
        # # in decoder
        # x = self.t5.decoder_state_embed(x) + self.t5.decoder_pos_embed[:, :T]
        # for blk in self.t5.decoder_blocks:
        #     x = blk(x, self.attn_mask)
...
class Critic(nn.Module):
    ...
    def forward(self, obs_seq, action_seq, target=False):
        if len(obs_seq.size())==2:
            ...
            #print("after unsqueeze:",obs_seq.size())
        batch_size, T, obs_dim = obs_seq.size()
        if len(action_seq.size())==2:
            #print()
            batch_size, act_dim = action_seq.size()
            action_seq = action_seq.view(batch_size,1,act_dim)
            #print("after unsqueeze:",obs_seq.size())
        state = self.t5.state_embed(obs_seq)
        action = self.t5.action_embed(action_seq)
        x = torch.stack([state, action], dim=1).permute(0, 2, 1, 3).reshape(batch_size, 2*T, self.config.n_embd)
        x += self.t5.pos_embed[:, :2*T]
        # encoder
        for blk in self.t5.encoder_blocks:
            x = blk(x, self.attn_mask)
        #x = self.t5.encoder_norm(x)
        #
        # # decoder preprocess
        # s = self.t5.decoder_state_embed(x[:, ::2])
        # a = self.t5.decoder_action_embed(x[:, 1::2])
        # x = torch.stack([s, a], dim=1).permute(0, 2, 1, 3).reshape_as(x)
        # x += self.t5.decoder_pos_embed[:, :2*T]
        # # decoder
        # for blk in self.t5.decoder_blocks:
        #     x = blk(x, self.attn_mask)
        if target is True:
...
class MTT5Agent:
    def __init__(self,
        ...
        if path is not None:
            self.actor.t5.load_state_dict(payload['model'])
            self.critic.t5.load_state_dict(payload['model'])
        # optimizers
    ...
    def act(self, obs, global_step, eval_mode,meta=None):
